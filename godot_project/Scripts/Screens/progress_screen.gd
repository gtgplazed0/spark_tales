...
func _process(delta: float) -> void:
	pass
func create_sticker_textures(num_of_levels):
	for i in num_of_levels:
		var sticker_container = TextureRect.new()
		sticker_container.name = "sticker_container_" + str(i+1)
...
func create_sticker_array():
	...
	if sticker_directory:
		...
		# Check if it's a file (and not a folder) and has an image extension
			if !sticker_directory.current_is_dir() and file_name.ends_with(".png") or file_name.ends_with(".jpg") or file_name.ends_with(".webp"):
		# Load the texture and add it to the array
				...
				print("not a story file, or a directory. ")
